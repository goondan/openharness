...
### 3. `control.abortConversation()`은 실행 중인 turn을 중단하는 공식 primitive

- `control.abortConversation({ conversationId, agentName?, reason? })`
  - 현재 conversation에서 실행 중인 turn이 있으면 `AbortSignal`을 통해 즉시 중단을 요청합니다.
  - `reason`은 `AbortError.message`와 `abortSignal.reason`으로 전달됩니다.
  - turn/step/toolCall middleware와 tool handler는 모두 `ctx.abortSignal`로 중단 여부를 확인할 수 있습니다.
  - LLM 호출과 tool 실행도 같은 신호를 공유하므로, host가 별도 우회 구현 없이 stop 요청을 내릴 수 있습니다.
...
- `createHarnessRuntimeFromYaml()` 반환값: `{ processTurn, ingress, control, close }`
...
- `persistence`를 주지 않으면 기존 파일 기반 저장을 사용합니다.
- `FileWorkspacePersistence`는 기본 file-backed 구현체이고, 다른 저장소는 `WorkspacePersistence` 포트를 구현해 주입할 수 있습니다.
...
- [OPENHARNESS_WORKSPACE_PERSISTENCE_SPEC.md](./OPENHARNESS_WORKSPACE_PERSISTENCE_SPEC.md): workspace persistence 포트와 file-backed 기본 구현 스펙
