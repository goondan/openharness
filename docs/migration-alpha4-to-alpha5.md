# Migration Guide: `0.0.1-alpha4` -> `0.0.1-alpha5`

이 문서는 _alpha4를 이미 쓰고 있는 사람_ 이 alpha5로 옮길 때 필요한 변경만 정리합니다.

결론부터 말하면, alpha5는 **사실상 v2 line** 이라서 "몇 군데 고치면 끝" 수준이 아닙니다.
...
즉, alpha4 -> alpha5는 _설정 번역_ 이 핵심이고, 대부분의 경우 점진적 패치보다 한 번에 옮기는 편이 낫습니다.
...
- package 구조: alpha4의 manifest/integrations 중심 구조 -> alpha5의 code-first 패키지 구조
...
alpha5에서는 `connections` 아래 plain object로 정의합니다.
...
alpha4 말기에 이미 `Message.data: ModelMessage`와 `metadata.__createdBy` 방향이 도입되었기 때문에, **그 부분은 alpha4 최신 사용법을 이미 따라가고 있었다면 큰 추가 수정이 없을 수 있습니다.**

하지만 alpha5에서는 이 메시지 envelope이 v2 실행 경로 전반의 기본 전제가 되었으므로, 아래 가정에 기대고 있던 코드는 다시 확인해야 합니다.

- `message.role`, `message.content`에 직접 접근하는 코드
- 메시지 provenance를 별도 `source` 필드로 추적하는 코드
- tool call/result 파트 이름을 자체 포맷으로 가정하는 코드

alpha5에서는 다음 구조를 기준으로 생각하는 게 안전합니다.
...
이 경우 alpha5를 alpha4의 "다음 버전"이라기보다, **같은 문제를 더 단순한 code-first API로 다시 푼 v2 line** 으로 받아들이는 편이 현실적입니다.
