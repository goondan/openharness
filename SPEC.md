...
ingress 이벤트는 workspace runtime log에 남기고, conversationId가 확정된 이벤트는 conversation runtime log에도 남긴다.
...
9. `Agent.tools` / `Agent.extensions`가 생략되거나 빈 배열이어도 base tool bundle이나 특정 extension이 자동 활성화되지 않는다.
10. `content[]`의 conversation message 투영과 system prompt 주입은 선택된 extension이 소유하며, core runtime은 fallback middleware를 자동 설치하지 않는다.
11. `processTurn(text)`는 `source.kind = "connector"`, `source.name = "cli"`인 synthetic event로 runtime inbound 표면과 합류한다.
12. outbound 공통 포트는 추가하지 않는다. 채널별 응답은 tool/extension 책임으로 남는다.
