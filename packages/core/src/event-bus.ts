 ...
 *
 * This bus is the observation layer (HarnessEvents). It is distinct from the
 * conversation event log (MessageEvent via `conversation.append`/`getEventLog`):
 * `conversation.append` never calls this bus.
 */
...
export class EventBus {
  ...
  emit<T extends keyof HarnessEvents>(event: T, payload: HarnessEvents[T]): void {
    const key = event as string;
