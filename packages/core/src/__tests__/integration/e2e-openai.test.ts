 ...
 *
 * These tests make actual API calls and are skipped unless OPENAI_API_KEY is set.
 * Run with: OPENAI_API_KEY=... npx vitest run packages/core/src/__tests__/integration/e2e-openai.test.ts
 */
...
describeE2E("E2E: OpenAI API", () => {
  ...
  it("simple text response without extensions", async () => {
    const config: HarnessConfig = {
      agents: {
        default: {
          model: { provider: "openai", model: "gpt-4o-mini", apiKey: API_KEY! },
        },
      },
    };
  ...
  it("multi-turn conversation preserves context", async () => {
    ...
    const runtime = await createHarness(config);
    const convId = "e2e-openai-multi-turn";
