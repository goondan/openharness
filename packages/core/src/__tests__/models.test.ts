...
describe("AI SDK adapter chat()", () => {
  ...
  it("keeps system messages in generateText messages and suppresses AI SDK warnings", async () => {
    ...
    const passedMessages = capturedArgs!["messages"] as Array<{
      role: string;
      content: string;
    ...
    expect(passedMessages[1]).toEqual({ role: "system", content: "Follow project policy" });
    expect(passedMessages[2]).toEqual({ role: "user", content: "Hi" });
  });
