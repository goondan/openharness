 ...
 * step.
 *
 * As of 1.0 this is a *projection* (F2): the system prompt is part of the
 * throwaway prompt view, never the durable log. If this projection ran zero
 * times the durable log would still be correct — which is exactly the test for
 * "view, not mutation". A one-time turn middleware (phase `observe`, so it lands
 * before any step) removes the legacy persisted copy from conversations created
 * by older versions.
 */
export function BasicSystemPrompt(text: string): BasicSystemPromptExtension {
  return {
    ...
    register(api: AgentExtensionApi): void {
      ...
      // durable log and the projected view don't carry it twice.
      api.pipeline.register(
        "turn",
        async (ctx, next) => {
          const hasLegacy = ctx.conversation.messages.some(
            (m) => m.id === LEGACY_SYSTEM_MESSAGE_ID,
          );
          if (hasLegacy) {
      ...
      // Prompt projection: prepend the system message to the view. Filtering an
      // existing copy first keeps the projection idempotent and collision-free
      // even if the legacy message is still present on the first step.
      api.prompt.transform("basic-system-prompt", (view) => {
        ...
        return [system, ...rest] as PromptView;
      });
    },
  };
}
