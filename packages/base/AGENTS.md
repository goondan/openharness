...
5. Extension은 Runtime이 제공하는 `ctx.runtime.agent`/`ctx.runtime.inbound` 실행 컨텍스트를 활용하되, 프롬프트 본문 정책의 최종 소유권은 Extension에 둔다.
...
- `context-message`의 `runtime_route` 요약은 connector inbound 기준으로만 생성한다.
