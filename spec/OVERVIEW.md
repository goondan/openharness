...
| LLM 스트리밍 (EventBus 기반 관찰) | **Stable** | 스트리밍은 관찰 관심사. TurnResult/StepResult 변경 없이 step.textDelta/step.toolCallDelta 이벤트로 전달. streamChat 미구현 시 chat() 폴백 보장. |
...
| FR-CORE-007 | Turn 시작 시 코어가 인바운드 메시지를 대화 상태에 append 이벤트로 자동 추가한다. 이것이 코어가 대화 상태에 직접 쓰는 유일한 동작이다 (LLM 응답/도구 결과 append 포함). | G-1 |
| FR-CORE-008 | TurnContext에 현재 에이전트의 모델 구성에 바인딩된 LlmClient를 `llm` 필드로 노출한다. Extension은 런타임에 이를 통해 LLM 호출이 가능하다. | G-1, G-2 |
| FR-CORE-009 | LlmClient.chat()은 선택적 4번째 인자 LlmChatOptions를 받아 호출별로 model, temperature, maxTokens를 오버라이드할 수 있다. | G-2 |
| FR-CORE-010 | LlmClient는 선택적 `streamChat()` 메서드를 가진다. `streamChat()`은 `chat()`과 동일한 `Promise<LlmResponse>`를 반환하되, 스트리밍 중 콜백(`LlmStreamCallbacks`)을 호출하여 텍스트/도구호출 델타를 실시간으로 전달한다. `streamChat()`이 없으면 코어는 `chat()`으로 폴백한다. | G-1, G-2 |
...
| FR-OBS-004 | 코어가 발행하는 이벤트: turn.start, turn.done, turn.error, step.start, step.done, step.error, step.textDelta, step.toolCallDelta, tool.start, tool.done, tool.error, ingress.received, ingress.accepted, ingress.rejected. | G-1 |
...
| FR-EXT-004 | `api.pipeline` — 실행 미들웨어(Turn/Step/ToolCall) 및 Ingress 미들웨어(verify/normalize/route/dispatch) 등록. | G-2 |
...
| FR-API-002 | `runtime.processTurn(agentName, input, options?)`으로 Turn을 실행한다. `options.conversationId`로 대화를 이어간다. 호출자는 Turn 완료까지 await한다 (Ingress의 fire-and-forget과 대비). | G-1 |
| FR-API-003 | `runtime.ingress.receive()`로 Connector의 전체 파이프라인을 실행한다. fan-out 결과를 배열로 반환한다. | G-1 |
| FR-API-004 | `runtime.ingress.dispatch({ connectionName, envelope })`로 이미 정규화된 InboundEnvelope를 직접 접수한다 (verify/normalize 건너뜀). | G-1 |
...
- **Given** harness.config.ts에 model과 Extension 1개(BasicSystemPrompt)가 선언되어 있고, Tool은 선언하지 않았다.
...
- **Then** LLM에 시스템 프롬프트 없이 사용자 입력만 전달되고, 도구도 없다. 빈 컨텍스트에서 Turn이 실행된다.
...
### AC-15: Extension에서 LLM 호출

- **Given** Turn 미들웨어를 등록한 Extension이 있다.
- **When** 미들웨어 내에서 `ctx.llm.chat(messages)`를 호출한다.
- **Then** 현재 에이전트의 모델 구성으로 LLM 호출이 실행되고, LLM 응답이 반환된다.

### AC-16: LlmChatOptions를 통한 호출별 오버라이드

- **Given** Extension이 `ctx.llm`을 통해 LLM 호출을 수행하려 한다.
- **When** `ctx.llm.chat(messages, [], [], { model: "다른모델", temperature: 0 })`처럼 LlmChatOptions를 전달한다.
- **Then** 해당 호출에서만 지정된 옵션이 적용되고, 에이전트의 기본 모델 구성은 변경되지 않는다.

### AC-17: 스트리밍 — 텍스트 델타 이벤트

- **Given** `streamChat()`을 구현한 LlmClient가 바인딩된 에이전트에서 Turn을 실행한다. `api.on("step.textDelta", listener)`가 등록되어 있다.
- **When** LLM이 텍스트 응답을 스트리밍한다.
- **Then** `step.textDelta` 이벤트가 `step.start`와 `step.done` 사이에 여러 번 발행되고, 각 이벤트의 `delta`를 이어 붙이면 최종 `StepResult.text`와 동일하다.

### AC-18: 스트리밍 — streamChat 미구현 시 폴백

- **Given** `streamChat()`이 없는 커스텀 LlmClient가 바인딩된 에이전트에서 Turn을 실행한다.
- **When** Step이 LLM을 호출한다.
- **Then** 코어는 `chat()`으로 폴백하여 정상적으로 Turn을 완료한다. `step.textDelta` 이벤트는 발행되지 않는다.

### AC-19: 스트리밍 — 스트림 중 abort

- **Given** `streamChat()`을 구현한 LlmClient가 바인딩된 에이전트에서 Turn을 실행 중이다.
- **When** 스트리밍 도중 `control.abortConversation()`을 호출한다.
- **Then** AbortSignal이 전파되어 스트림이 중단되고, Turn이 `aborted` 상태로 종료된다.
...
| ~~Streaming/SSE 응답~~ | ~~v2 MVP 이후 검토.~~ → **Phase 2로 구현됨 (FR-CORE-010, FR-OBS-004).** EventBus 기반 스트리밍으로 `step.textDelta`/`step.toolCallDelta` 이벤트 발행. TurnResult/StepResult에는 변경 없음. |
...
| LLM 접근 | 유닛 테스트: TurnContext.llm으로 Extension 내 LLM 호출, LlmChatOptions 오버라이드 | AC-15, AC-16 통과 |
| 스트리밍 | 유닛 테스트: streamChat 어댑터 콜백, executeStep 스트리밍 이벤트 발행, chat() 폴백, 스트림 abort | AC-17, AC-18, AC-19 통과 |
