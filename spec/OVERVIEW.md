...
| Ingress Disposition | ingress accepted 결과가 새 Turn을 시작했는지(`started`), 기존 active Turn에 합류했는지(`steered`), pending HITL batch queue에 저장됐는지(`queuedForHitl`) 나타내는 값 |
...
| FR-INGRESS-008 | route 결과의 `(agentName, conversationId)`에 steer 가능한 active turn이 있고 open HITL batch가 없으면 새 Turn 대신 해당 Turn의 다음 Step 입력으로 접수한다. |
| FR-INGRESS-009 | `IngressAcceptResult.disposition`은 새 Turn 시작이면 `started`, 기존 active Turn 합류면 `steered`, pending HITL batch queue 저장이면 `queuedForHitl`이다. |
| FR-INGRESS-010 | route 결과의 `(agentName, conversationId)`에 open HITL batch가 있으면 새 Turn을 시작하지 않고 durable steer queue에 저장한다. |
...
| FR-HITL-004 | runtime은 pending HITL batch와 request 조회 API를 제공한다. |
...
| FR-HITL-015 | `HitlBatch`는 기본적으로 resume에 필요한 conversation event snapshot을 포함한다. |
| FR-HITL-016 | 한 Step에 여러 HITL ToolCall이 있으면 하나의 batch 안에 여러 pending request로 저장한다. |
| FR-HITL-017 | 같은 Step의 non-HITL ToolCall은 실행되고 그 결과가 durable batch record에 저장된다. |
| FR-HITL-018 | batch의 모든 HITL request가 resolved/rejected 되기 전에는 resume을 시작하지 않는다. |
| FR-HITL-019 | pending HITL batch가 있는 conversation의 steer 입력은 durable queue에 저장된다. |
| FR-HITL-020 | batch resume은 tool-result flush 후 queued steer를 drain하고 다음 Step 전에 반영한다. |
...
| NFR-HITL-001 | HITL 내구성: `hitl.requested` 이벤트 전 pending batch/request가 durable store에 저장된다. | crash recovery test |
| NFR-HITL-002 | HITL 멱등성: 같은 request/result 제출 또는 같은 batch resume 재시도로 tool handler와 conversation append가 중복 실행되지 않는다. | concurrency test |
| NFR-HITL-003 | HITL 복구성: runtime 재생성 후 기존 pending batch/request와 queued steer를 조회하고 재개할 수 있다. | integration test |
| NFR-HITL-004 | HITL 관찰성: 모든 HITL 상태 전이는 `batchId`, `requestId`, `turnId`, `toolCallId`, `conversationId`를 포함한다. | unit test |
...
| NFR-HITL-007 | HITL retention: 완료/거절/만료된 batch/request는 store별 retention 정책으로 정리 가능해야 한다. | adapter test |
| NFR-HITL-008 | HITL ordering: batch tool-result flush와 queued steer drain은 deterministic ordering을 유지한다. | ordering test |
| NFR-HITL-009 | HITL identity: `batchId`와 `requestId`는 trace field에서 파생되지 않는 opaque generated ID다. | unit test |
...
| HITL batch 상태 머신과 store 계약 | Stable | 외부 승인 UI와 durable adapter가 의존 |
...
| `core/hitl.md` | durable HITL batch/request, human result 제출, queued steer, crash recovery, batch resume |
...
- Given 같은 `(agentName, conversationId)`의 Turn이 실행 중이고 open HITL batch가 없다
...
- Given Turn이 `turn.done` 또는 `turn.error`를 발행하는 중이고 open HITL batch가 없다
...
### AC-07 HITL batch pending 전환

- Given HITL required tool이 등록되어 있고 LLM이 해당 도구를 호출하면
- When Step이 ToolCall batch를 처리하면
- Then pending HITL batch/request가 durable store에 저장되며 Turn은 `waitingForHuman`으로 종료된다.

### AC-07b HITL mixed tool batch

- Given 한 Step에 normal ToolCall과 여러 HITL ToolCall이 함께 있다
- When batch preparation이 완료된다
- Then normal ToolCall은 실행되어 durable batch result로 기록되고 HITL ToolCall은 같은 `batchId`의 pending request가 되며 그 이후에만 batch가 `waitingForHuman`으로 노출된다.

### AC-08 HITL 재시작 복구

- Given pending HITL batch/request와 queued steer가 durable store에 저장되어 있으면
- When runtime을 종료하고 새 runtime을 생성한 뒤 pending HITL을 조회하면
- Then 같은 `batchId`와 pending `requestId`가 조회되고 queued steer가 보존된다.

### AC-09 HITL peer 대기

- Given 같은 batch에 pending HITL request가 2개 이상 있다
- When 하나의 request에 approve/reject result를 제출한다
- Then result는 durable 저장되고 submit result는 `resume.status="waitingForPeers"`를 반환한다.

### AC-09b HITL batch 승인 재개

- Given batch의 마지막 pending HITL request에 approve/reject result를 제출하면
- When in-runtime resume task가 실행되면
- Then batch의 모든 tool-result가 `toolCallIndex` 순서로 conversation에 기록되고 queued steer가 그 뒤에 반영된 뒤 continuation Step이 실행된다.

### AC-10 HITL 중복 방지

- Given 같은 request에 같은 result가 여러 번 제출되거나 같은 batch resume이 동시에 실행되면
- When lease/fencing guard가 적용된다
- Then tool handler와 batch tool-result flush는 최대 1회만 수행된다.

### AC-10b HITL append crash recovery

- Given HITL resume이 tool-result를 conversation에 append한 뒤 `commitBatchAppend()` 전에 crash가 발생한다
- When startup recovery가 같은 batch resume을 재시도한다
- Then stable event id 때문에 tool-result와 queued steer는 conversation에 중복 append되지 않고 batch completion이 완료된다.

### AC-10c HITL continuation crash recovery

- Given HITL resume이 `commitBatchAppend()`를 완료한 뒤 continuation 시작 전에 crash가 발생한다
- When startup recovery가 실행된다
- Then 저장된 continuation `turnId`로 continuation Step이 schedule된다.
...
### AC-13 HITL 대기 중 steer queue

- Given pending HITL batch가 있는 conversation이다
- When 같은 route 결과를 가진 ingress가 들어온다
- Then 새 Turn을 시작하지 않고 input을 durable queue에 저장하며 `disposition="queuedForHitl"`을 반환한다.

### AC-14 HITL opaque identity

- Given batch와 request가 생성된다
- When `batchId`와 `requestId`를 검사하면
- Then 두 ID는 `turnId`, `stepNumber`, `toolCallId`에서 파생되지 않은 opaque generated ID다.

### AC-15 HITL blocked recovery

- Given external handler execution marker 이후 process crash가 발생한다
- When startup recovery가 실행된다
- Then 해당 batch/request는 자동 재실행되지 않고 operator recovery 대상 상태로 남는다.
...
- HITL integration test로 batch pending 전환, mixed tool batch preparation, runtime 재생성 후 조회, peer 대기, 승인/거절 batch resume, queued steer drain, concurrent resume race, append 후 commit 전 crash recovery, commit 후 continuation 전 crash recovery를 검증한다.
