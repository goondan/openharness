...
| Human Gate | ToolCall 실행 전 사람 승인/입력을 요구하며 conversation 진행을 막는 blocker |
...
| ID | Level | 요구사항 |
| --- | --- | --- |
| FR-HG-001 | Committed | ToolDefinition은 선택적으로 human gate policy를 선언할 수 있다. |
| FR-HG-002 | Committed | human gate가 필요한 ToolCall은 handler 실행 전에 durable Human Gate와 Human Task를 생성해야 한다. |
...
| FR-HG-016 | Committed | Human Gate resume은 같은 gate에 대해 at-most-once tool handler execution을 보장해야 한다. |
| FR-HG-017 | Planned | TTL, reminder, escalation, external approval UI는 extension/host 정책으로 구성할 수 있어야 한다. |
| FR-HG-018 | Planned | Human Gate resume 이후 자동 continuation Turn 실행은 durable conversation execution과 함께 확장한다. |
...
### AC-07 Human Gate pending 전환

- Given Human Gate required tool이 등록되어 있고 LLM이 해당 도구를 호출하면
