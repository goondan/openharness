...
현재 ingress는 verify/normalize/route 이후 즉시 in-memory Turn 실행 또는 active Turn steering으로 연결된다. direct `processTurn()`은 ingress와 별개로 즉시 Turn을 시작한다. 이 구조에서는 active Turn 중 들어온 input, Human Approval 대기 중 input, process crash 직전 accepted 된 input이 같은 durability/ordering/idempotency 계약을 공유하지 못한다.
...
| G-7 | 복구 가능한 Human Approval | 사람 승인이 필요한 ToolCall은 프로세스 재시작/크래시/장기간 대기 이후에도 안전하게 재개할 수 있어야 한다. |
...
| G-9 | 통합 conversation scheduling | idle, active Turn, Human Approval blocker, recovery 상황의 inbound input 처리는 같은 scheduler와 ordering/idempotency 계약을 따라야 한다. |
...
| S-3 | Human Approval 중 들어온 input은 Human Approval 전용 queue가 아니라 durable inbound item으로 blocked 상태가 된다. | Human Approval integration test |
...
| Human Approval Policy | ToolCall 실행 전 사람 승인/입력을 요구하는 public policy. `ToolDefinition.humanApproval`과 `HarnessConfig.humanApproval`로 선언한다. |
| Human Approval Lifecycle | Human Approval을 durable하게 표현하는 approval/blocker lifecycle. persisted blocker/event key는 `humanApproval`을 사용한다. |
| Human Task | Human Approval이 사람에게 노출하는 승인/거절/입력 요청 |
| Conversation Blocker | 특정 `(agentName, conversationId)`의 새 Turn/delivery를 막는 durable state. canonical owner는 Human Approval 또는 operator hold store이며 inbound item은 blocker reference만 저장한다. |
...
| FR-INGRESS-012 | Human Approval 또는 operator blocker가 있는 conversation의 ingress input은 새 Turn을 시작하지 않고 durable inbound item으로 blocked 상태가 되어야 한다. |
...
| FR-SCHED-003 | Committed | scheduler는 Human Approval blocker가 있으면 inbound item을 `blockedBy=humanApproval`로 표시하고 새 Turn을 시작하지 않아야 한다. |
...
### 7.6 Human Approval / HITL

| ID | Level | 요구사항 |
| --- | --- | --- |
| FR-HA-001 | Committed | ToolDefinition은 선택적으로 `humanApproval` policy를 선언할 수 있다. |
| FR-HA-002 | Committed | `humanApproval`이 필요한 ToolCall은 handler 실행 전에 durable Human Approval와 Human Task를 생성해야 한다. |
| FR-HA-003 | Committed | human task 저장 성공 전에는 handler를 호출하면 안 된다. |
| FR-HA-004 | Committed | Human Approval은 conversation blocker로 등록되어 같은 conversation의 새 Turn 시작을 막아야 한다. |
| FR-HA-005 | Committed | Human Approval 중 새 inbound event는 별도 HITL queue가 아니라 durable inbound item으로 저장되고 `blockedBy=humanApproval`로 표시되어야 한다. |
| FR-HA-006 | Committed | Human Task는 approval, rejection, text input, structured form input을 지원해야 한다. |
| FR-HA-007 | Committed | human result submit은 durable write 이후에만 accepted response와 lifecycle event를 반환해야 한다. |
| FR-HA-008 | Committed | duplicate submit은 idempotent하게 기존 result로 수렴해야 한다. |
| FR-HA-009 | Committed | 모든 required Human Task가 resolved/rejected되기 전에는 blocked inbound item을 continuation에 반영하면 안 된다. |
| FR-HA-010 | Committed | Human Approval resume은 blocker 유지 상태에서 tool result를 먼저 append하고 blocked inbound items를 deterministic order로 append한 뒤 blocker를 해제해야 한다. |
| FR-HA-011 | Committed | human rejection은 handler 호출 없이 rejection tool result로 conversation에 반영되어야 한다. |
| FR-HA-012 | Committed | Human Approval status는 `waitingForHuman`, `ready`, `resuming`, `completed`, `blocked`, `canceled`, `expired`, `failed`를 표현해야 한다. |
| FR-HA-013 | Committed | Human Task status는 `waitingForHuman`, `resolved`, `rejected`, `canceled`, `expired`를 표현해야 한다. |
| FR-HA-014 | Committed | runtime은 pending Human Task 조회, human result 제출, resume/cancel control API를 제공해야 한다. |
| FR-HA-015 | Committed | Human Approval resume은 프로세스 재시작 이후에도 가능해야 한다. |
| FR-HA-016 | Committed | Human Approval resume은 handler 실행 전 durable `handlerStartedAt` boundary를 기록하고, 그 이후 lease 만료 재획득으로 같은 handler를 자동 재실행하지 않아야 한다. |
| FR-HA-017 | Planned | TTL, reminder, escalation, external approval UI는 extension/host 정책으로 구성할 수 있어야 한다. |
| FR-HA-018 | Committed | Human Approval resume은 tool result와 blocked inbound items를 append/consume한 뒤 continuation Turn을 실행해야 한다. |
...
| NFR-HA-001 | Human Approval durable-before-handler: `humanApproval.created`/`humanTask.created` 이벤트 전 approval/task가 durable store에 저장되고 handler는 실행되지 않는다. | crash recovery/unit test |
| NFR-HA-002 | Human Approval blocker consistency: blocker 중 inbound item은 durable inbound queue에 blocked 상태로 남는다. | integration test |
| NFR-HA-003 | Human Approval idempotency: duplicate task result/approval resume이 중복 handler execution을 만들지 않는다. | concurrency test |
| NFR-HA-004 | Human Approval observability: lifecycle events는 `humanApprovalId`, `humanTaskId`, `turnId`, `toolCallId`, `conversationId`를 포함한다. | unit test |
| NFR-HA-005 | Human Approval security: human result 제출은 task identity와 agent/conversation scope 검증을 통과해야 한다. | negative test |
| NFR-HA-006 | Human Approval compatibility: Human Approval 미사용 도구의 실행 루프와 결과 구조는 기존 동작을 유지한다. | regression test |
| NFR-HA-007 | Human Approval retention: 완료/거절/만료된 task는 store별 retention 정책으로 정리 가능해야 한다. | adapter test |
...
| Human Approval 상태 머신과 store 계약 | Stable | 외부 승인 UI와 durable adapter가 의존 |
...
| Human Approval이 queue를 소유하지 않는 모델 | Stable | HITL과 durable ingress 통합의 핵심 결정 |
...
| Human Approval CLI UX | Candidate | core 계약 검증 뒤 확장 가능 |
...
| Human Approval blocker와 scheduler race | Medium | High | `createApproval + registerBlocker`를 HumanApprovalStore atomic boundary로 고정 | blocked 중 새 Turn 생성 |
...
| `core/hitl.md` | Human Approval, Human Task, blocker, human result 제출, crash recovery, ToolCall resume |
...
- Given conversation이 Human Approval에서 waiting 상태다
- When 같은 conversation으로 inbound event가 들어온다
- Then 새 Turn은 시작되지 않고 durable inbound item이 `blockedBy=humanApproval`로 저장된다.

### AC-07 Human Approval pending 전환

- Given `humanApproval` required tool이 등록되어 있고 LLM이 해당 도구를 호출하면
- When Step이 ToolCall에 도달하면
- Then tool handler는 호출되지 않고 pending Human Approval와 Human Task가 durable store에 저장되며 Turn은 `waitingForHuman`으로 종료된다.

### AC-08 Human Approval 재시작 복구
...
- Human Approval integration test로 pending 전환, runtime 재생성 후 조회, 승인/거절 resume, concurrent resume race를 검증한다.
...
- durable Human Approval은 core가 특정 저장소를 내장하지 않고 `HumanApprovalStore` 계약을 통해 외부 backend나 adapter가 제공한다.
