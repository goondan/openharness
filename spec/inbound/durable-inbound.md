...
- Given connector가 stable external id를 제공하지 않는다, When 같은 conversation에 같은 text의 서로 다른 inbound event가 다른 `receivedAt`으로 들어온다, Then runtime은 각 event를 서로 다른 idempotency key로 append한다.
