...
- non-terminal HITL barrier가 있는 동안 같은 conversation으로 들어온 input은 새 turn을 시작하지 않는다.
- HITL barrier가 input을 durable queue에 저장할 수 있는 상태이면, runtime은 store write 성공 후에만 input을 accepted로 반환한다.
- HITL barrier가 input을 durable queue에 저장할 수 없는 상태이면, runtime은 input을 rejected/error로 반환하고 active turn memory queue에 넣지 않는다.
- `commitBatchAppend()` 이후 실행 중인 continuation turn에 대한 normal active-turn steering은 HITL queued steer가 아니며, HITL durability guarantee 밖의 기존 active-turn behavior다.
...
Section 7의 state model이 canonical contract다.
...
- queueable HITL barrier 중 들어온 ingress/processTurn input은 durable queue에 저장되거나, 저장할 수 없으면 accepted/started로 반환되지 않는다.
- direct `processTurn()`도 HITL barrier를 우회해 새 LLM step을 시작하면 안 된다.
...
| Queued Steer | queueable HITL barrier가 있는 동안 같은 conversation으로 들어와 durable queue에 저장된 입력 |
...
| Side-effect Boundary | tool handler 또는 continuation turn처럼 외부 호출/도구 실행이 시작되어 automatic replay가 안전하지 않은 경계 |
| Queueable HITL Batch | 같은 conversation input을 durable queued steer로 받을 수 있는 HITL batch 상태 |
...
| FR-HITL-010 | Committed | queueable HITL barrier 중 ingress/steer는 durable queue에 저장되어야 한다. |
...
| FR-HITL-014 | Committed | `preparing`처럼 queueable하지 않은 pre-continuation HITL barrier 상태에서는 같은 conversation input을 accepted로 반환하면 안 된다. |
| FR-HITL-015 | Committed | side-effect boundary 이후 실패/크래시는 automatic retry가 아니라 blocked/operator recovery로 수렴해야 한다. |
| FR-HITL-016 | Planned | TTL, reminder, cancel policy는 extension으로 추가 가능해야 한다. |
| FR-HITL-017 | Candidate | CLI는 pending 조회와 submit/resume 명령을 제공할 수 있다. |
...
| NFR-HITL-007 | Committed | No memory-only acceptance under HITL barrier: HITL barrier가 있는 conversation에서 accepted input은 durable store에 있거나 명시적으로 rejected/error여야 한다. |
...
6. 모든 non-HITL result가 저장되면 runtime은 `HitlStore.markBatchWaitingForHuman()`을 호출한다.
...
- non-HITL handler 시작 후 실패/크래시는 자동 재실행하지 않는다. durable execution marker/result를 기준으로 `waitingForHuman`, `blocked`, 또는 non-retryable failed로 수렴한다.
- `preparing` 상태는 pending UI/API 또는 durable steer queue 대상으로 노출되는 안정 상태가 아니다.
- `preparing` 중 같은 conversation input은 accepted/steered로 반환하지 않는다. queueable durable state가 아니기 때문이다.
...
4. `resolveRequest()`/`rejectRequest()`는 다음을 하나의 atomic store method로 처리한다.
   - request result 저장
   - duplicate submit 감지
   - 남은 pending peer 계산
...
**Trigger:** non-terminal HITL barrier가 있는 conversation으로 input이 들어온다.

**Main Flow:**

1. runtime/ingress는 먼저 해당 conversation에 queueable HITL batch가 있는지 조회한다.
2. queueable batch가 있으면 새 turn을 시작하지 않는다.
3. input을 durable envelope로 만들 수 있으면 `HitlStore.enqueueSteer()`로 저장한다.
4. 저장 성공 후에만 `queuedForHitl` 또는 equivalent queued result를 반환한다.
5. queueable batch가 없지만 pre-continuation HITL barrier가 있으면 input을 rejected/error로 반환한다.
6. barrier가 없으면 기존 dispatch/active-turn steering 규칙을 사용한다.
7. `continuing` batch가 있고 active continuation turn이 살아 있으면 runtime은 기존 active-turn steering을 사용할 수 있다. 이것은 HITL queued steer가 아니다.
8. `continuing` batch가 있지만 active continuation turn이 없으면 input을 rejected/error로 반환한다.
9. durable envelope를 만들 수 없거나 queue write가 실패하면 input을 accepted/steered/started로 반환하지 않는다.
...
- `preparing` batch는 같은 conversation의 중복 batch 생성을 막는 barrier지만, user input을 durable queue로 받는 대상은 아니다.
- `preparing` 중 input은 memory-only active turn steering으로 accepted 처리하지 않는다.
- `continuing` batch는 durable HITL queue를 열지 않는다. active continuation turn이 없는 경우 input은 rejected/error다.
...
- batch status가 `ready` 또는 side-effect boundary 이전의 retryable resume state다.
...
9. runtime은 `HitlStore.commitBatchAppend()`를 호출해 appended result와 drained steer를 commit한다.
10. `commitBatchAppend()` 성공 직후 batch는 `continuing`으로 전환되고, automatic resume retry 대상에서 제외된다.
11. runtime은 continuation step을 실행한다.
12. continuation이 successful terminal state로 settle되면 `HitlStore.completeBatch()`를 호출한다.
13. continuation이 error/abort 또는 unsafe state로 settle되면 batch는 `blocked` 또는 non-retryable failed로 수렴한다.
...
- `commitBatchAppend()` 실패 시 accepted queued steer는 삭제되거나 drained로 숨겨지면 안 된다. retry 시 같은 queued steer를 다시 반영할 수 있어야 한다.
- `commitBatchAppend()` 성공 후 continuation을 자동 재실행하면 안 된다. 이 시점부터 continuation side effect가 발생했을 수 있기 때문이다.
- continuation failure 또는 `completeBatch()` failure는 `completed`로 위장하면 안 된다. runtime은 batch를 `blocked` 또는 non-retryable failed로 남겨 operator recovery가 가능하게 한다.
- rejected request처럼 handler를 시작하지 않는 경로의 transient persistence failure는 automatic retry 가능하다. retry가 외부 handler side effect를 반복하지 않기 때문이다.
...
4. `resuming` 또는 retryable failed batch는 side-effect boundary를 넘지 않았다는 durable evidence가 있을 때만 resume task로 schedule할 수 있다.
5. `continuing` batch는 자동 resume하지 않는다. append commit 이후 continuation side effect가 발생했을 수 있기 때문이다.
6. `blocked` batch는 자동 resume하지 않는다.
7. `preparing` batch는 어떤 tool handler도 자동 실행하지 않고 durable records만으로 수렴시킨다.
8. `preparing` batch에 필요한 non-HITL tool results가 모두 있으면 `waitingForHuman`으로 수렴시킬 수 있다.
9. `preparing` batch에 execution marker가 있지만 result가 없으면 `blocked` 또는 non-retryable failed로 수렴한다.
10. `preparing` batch에 execution marker/result가 없으면 unexposed incomplete batch로 cancel할 수 있다.
...
- startup recovery는 `preparing` batch를 무조건 cancel하지 않는다.

### 6.7 Safety invariants

- Accepted input invariant: HITL barrier가 있는 conversation에서 accepted/steered/started로 반환된 input은 durable store에 있어야 한다.
- Non-queueable barrier invariant: `preparing`, `blocked`, terminal state는 user input을 memory-only queue로 받아들이지 않는다. `continuing`은 durable HITL queue가 아니며 active continuation turn이 없으면 input을 받지 않는다.
- Handler replay invariant: `startRequestExecution()` 또는 equivalent side-effect marker 이후에는 해당 handler를 automatic retry로 다시 호출하지 않는다.
- Continuation replay invariant: `commitBatchAppend()` 성공 이후 continuation은 automatic retry로 다시 실행하지 않는다.
- Completion invariant: continuation이 `error` 또는 `aborted`로 끝난 batch는 `completed`가 아니다.
- Preparing recovery invariant: `preparing` recovery는 durable records만 읽어서 상태를 수렴시키며 tool handler를 호출하지 않는다.
...
preparing -> failed | blocked | canceled
...
| Status | Meaning |
| --- | --- |
| `preparing` | batch/request는 생성됐지만 pending으로 노출할 안정 지점은 아직 아님. queueable이 아님 |
...
| `continuing` | resume append가 commit되고 continuation side-effect boundary를 넘었거나 넘을 수 있음. automatic retry 대상이 아님 |
...
- non-retryable terminal/failed batch는 open batch conflict 대상이 아니다.
- `markBatchWaitingForHuman()`은 batch/request와 필요한 non-HITL results가 저장된 경우에만 성공한다.
- `resolveRequest()`/`rejectRequest()`는 request result 저장과 batch ready 전환을 같은 atomic operation으로 처리한다.
...
- `commitBatchAppend()` 실패 전까지 queued steer는 retry 가능한 상태로 남아야 한다.
...
- Ingress owns routing input into `queuedForHitl` instead of starting a new turn when a queueable HITL batch exists, and rejecting input when a HITL barrier is not queueable.
- Store adapter owns atomicity, durability, idempotent submit, open-batch uniqueness, queued steer retention, resume append commit, and side-effect boundary markers.
...
- Given queueable pending HITL exists for a conversation, When ingress targets the same conversation, Then it is stored as queued steer and no new turn starts.
...
- Given pending HITL exists for a conversation, When direct `processTurn()` targets the same conversation, Then it must durably queue or reject/error without appending a new user message or starting a new LLM step.
...
- Given commit append has succeeded and continuation starts, When continuation fails or the process crashes before batch completion, Then recovery does not automatically rerun continuation and the batch is blocked or non-retryable failed.
...
- Given a `preparing` batch has a non-HITL execution marker without result, When startup recovery runs, Then it becomes blocked/non-retryable and no handler is rerun.
...
- `resolveRequest()`/`rejectRequest()` duplicate/idempotent behavior
