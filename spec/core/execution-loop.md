...
- Main Flow:
  ...
  8. approval 경로에서 `HumanApprovalPendingError`가 발생하면 즉시 상위 Turn 루프로 전파한다. 병렬 경로에서 handler가 동적으로 `HumanApprovalPendingError`를 던지면 settle된 sibling result를 단일 tool 메시지로 기록한 뒤 첫 rejection을 전파한다 — 단, 표준 패턴은 tool 정의의 `humanApproval`을 선언하는 것이다.
...
- 하나의 assistant tool-call batch에 대한 tool-result는 하나의 `role="tool"` 메시지에 모아 append한다. 같은 assistant batch의 sibling tool-result를 여러 tool 메시지로 나누면 provider adapter의 직전 메시지 인접성 계약을 깨므로 코어에서 허용하지 않는다.
- 병렬 경로에서 handler/middleware가 `humanApproval` 정적 선언 없이 `HumanApprovalPendingError`를 던지는 동적 HITL은 **지원되지 않는다.** step.ts는 settle된 sibling의 tool-result는 LLM 순서대로 단일 tool 메시지에 append하고 LLM 순서상 첫 rejection을 전파하지만, 같은 step에서 두 개 이상의 동적 approval gate가 생성되면 두 번째 이후의 gate는 orphan 상태가 된다. 동적 approval을 쓰려는 extension은 해당 tool의 `ToolDefinition.humanApproval`도 같이 선언해 pre-flight가 approval 경로로 라우팅하도록 만들어야 한다.
